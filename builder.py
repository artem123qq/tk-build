# ============================================================
# TkBuilder Ultra v9.2 — Финальный рефактор: все ошибки из ревью исправлены
import weakref
import time
...
def _make_window(parent=None, title="", geometry="500x400", bg=None):
    """Создаёт правильно настроенное дочернее окно."""
    ...
    try:
        w, h = int(geometry.split('x')[0]), int(geometry.split('x')[1].split('+')[0])
...
#
    def create_widget_instance(self, wtype, props=None, canvas=None):
        """Расширенный метод создания виджетов с поддержкой плагинов"""
        p = props or {}
        c = canvas or self.builder.canvas
#
        # Если это виджет из плагина
        if wtype.startswith("plugin:"):
            widget_id = wtype.replace("plugin:", "")
            widget_instance = self.create_widget_instance(widget_id, props, canvas)
            if widget_instance:
...
#
        ...
        try:
            if wtype == "Button":
                return tk.Button(c, text=p.get("text", "Кнопка"),
                                 bg=p.get("bg", "#4CAF50"), fg=p.get("fg", "white"),
                                 width=10, height=1)
#
            elif wtype == "Label":
                return tk.Label(c, text=p.get("text", "Метка"),
                                bg=p.get("bg", "#2196F3"), fg=p.get("fg", "white"),
                                width=10, height=1)
#
            elif wtype == "Entry":
                e = tk.Entry(c, bg=p.get("bg", "white"), fg=p.get("fg", "black"),
                             width=20)
                if p.get("text"):
...
#
                ...
                t = tk.Text(c, width=40, height=10,
...
#
                ...
                cb = tk.Checkbutton(c, text=p.get("text", "Флажок"),
                                    variable=var, bg=c.cget("bg"))
                cb._var = var
                return cb
#
            elif wtype == "Radiobutton":
                var = tk.IntVar(value=p.get("value", 0))
                rb = tk.Radiobutton(c, text=p.get("text", "Радио"),
                                    variable=var, value=1, bg=c.cget("bg"))
                rb._var = var
                return rb
#
            elif wtype == "Listbox":
                lb = tk.Listbox(c, height=6, width=20)
                for item in p.get("items", ["Элемент 1", "Элемент 2"]):
                    lb.insert("end", item)
                return lb
#
            elif wtype == "Combobox":
                cb = ttk.Combobox(c, values=p.get("items", ["Вариант 1", "Вариант 2"]),
                                  width=17)
                if p.get("text"):
                    cb.set(p.get("text"))
                return cb
#
                ...
                return tk.Scale(c, from_=0, to=100, orient="horizontal", length=150)
#
            elif wtype == "Progressbar":
                return ttk.Progressbar(c, length=150, value=p.get("value", 50))
#
            elif wtype == "Menu":
                lbl = tk.Label(c, text="[МЕНЮ]", bg="#FF9800", fg="white",
...
#
                ...
                tree = ttk.Treeview(c, columns=("value",), show="tree headings", height=6)
...
#
                ...
                return tk.Spinbox(c, from_=0, to=100, width=10)
#
            elif wtype == "Switch":
                # Создаем кастомный переключатель
                frame = tk.Frame(c, bg=p.get("bg", "#f0f0f0"), width=60, height=30)
...
#
                ...
                card = tk.Frame(c, bg="white", relief="raised", bd=2, width=180, height=120)
#
                # Заголовок карточки
...
#
                ...
                # Бейдж с текстом
                badge = tk.Label(c, text=p.get("text", "Бейдж"),
...
#
                ...
                # Уведомление
                notif = tk.Frame(c, bg="#FFEB3B", relief="solid", bd=1, width=200, height=40)
#
...
class WidgetInspector(tk.Frame):
    """Инспектор виджетов — показывает реальные свойства"""
#
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        bg = parent.cget("bg")
        self.configure(bg=bg, width=200, height=160)
#
...
#
        self._rows = {}
        fields = ["Тип", "X", "Y", "Ширина", "ID"]
        for field in fields:
            row = tk.Frame(self, bg=bg)
            row.pack(fill="x", padx=5)
            tk.Label(row, text=field+":", bg=bg, fg="#aaa",
                     font=("Segoe UI", 8), width=8, anchor="w").pack(side="left")
            val = tk.Label(row, text="—", bg=bg, fg="#00ff88",
                           font=("Segoe UI", 8), anchor="w")
            val.pack(side="left", fill="x", expand=True)
            self._rows[field] = val
#
    def update_info(self, widget_info):
        if not widget_info:
            for v in self._rows.values():
                v.config(text="—")
            return
        self._rows["Тип"].config(text=widget_info.get("type", "?"))
        self._rows["X"].config(text=str(int(widget_info.get("x", 0))))
        self._rows["Y"].config(text=str(int(widget_info.get("y", 0))))
        self._rows["Ширина"].config(text=str(widget_info.get("props", {}).get("width", "auto")))
        self._rows["ID"].config(text=str(widget_info.get("id", "?")))
#
#
class PerformanceMonitor(tk.Frame):
    """Монитор производительности — реальная статистика"""
#
    def __init__(self, parent, **kwargs):
        super().__init__(parent, **kwargs)
        bg = parent.cget("bg")
        self.configure(bg=bg, width=200, height=120)
...
#
        self.widgets_label = tk.Label(self, text="Виджетов: 0",
                                      bg=bg, fg="#4CAF50", font=("Segoe UI", 8))
        self.widgets_label.pack()
        self.mem_label = tk.Label(self, text="Память: —",
                                  bg=bg, fg="#2196F3", font=("Segoe UI", 8))
        self.mem_label.pack()
        self.uptime_label = tk.Label(self, text="Время: 0с",
                                     bg=bg, fg="#FF9800", font=("Segoe UI", 8))
        self.uptime_label.pack()
        self._tick()
#
    def update_stats(self, widget_count):
        self.widgets_label.config(text=f"Виджетов: {widget_count}")
#
    def _tick(self):
        try:
            import os, sys
            uptime = int(time.time() - self._start)
            self.uptime_label.config(text=f"Время: {uptime}с")
            try:
                import resource
                mem = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss // 1024
                self.mem_label.config(text=f"Память: {mem} МБ")
            except Exception:
                self.mem_label.config(text="Память: N/A")
...
#
    def export_html(self):
        """Реальный экспорт в HTML"""
        # Ищем builder в родительской иерархии
        builder = None
        parent = self.master
        while parent:
            if hasattr(parent, 'widgets_info'):
                builder = parent
                break
            parent = getattr(parent, 'master', None)

        if not builder or not builder.widgets_info:
            messagebox.showwarning("Экспорт HTML", "Нет виджетов для экспорта")
            return
        ...
        html_lines = [
            ...
            "  input[type=text] { border: 1px solid #ccc; border-radius: 4px; padding: 4px 8px; }",
            "  textarea { border: 1px solid #ccc; border-radius: 4px; padding: 4px 8px; }",
            "  select { border: 1px solid #ccc; border-radius: 4px; padding: 4px 8px; }",
            "</style>",
            "</head>",
            "<body>"
        ]

        for info in builder.widgets_info:
            t = info["type"]
            p = info["props"]
            x, y = int(info["x"]), int(info["y"])
            bg = p.get("bg", "")
            fg = p.get("fg", "")
            text = p.get("text", "").replace("'", "&apos;").replace('"', "&quot;")
            ...
            if t == "Button":
                w = p.get("width", 80)
                html_lines.append(f'  <button class="widget" style="{style} width:{w*7}px;">{text}</button>')
            elif t == "Label":
                html_lines.append(f'  <span class="widget" style="{style}">{text}</span>')
            elif t == "Entry":
                html_lines.append(f'  <input type="text" class="widget" style="{style}" placeholder="{text}">')
            elif t == "Text":
                html_lines.append(f'  <textarea class="widget" style="{style}" rows="5" cols="30">{text}</textarea>')
            elif t == "Checkbutton":
                html_lines.append(f'  <label class="widget" style="{style}"><input type="checkbox"> {text}</label>')
            elif t == "Radiobutton":
                html_lines.append(f'  <label class="widget" style="{style}"><input type="radio"> {text}</label>')
            elif t == "Combobox":
                items = p.get("items", [])
                opts = "".join(f"<option>{item}</option>" for item in items)
                html_lines.append(f'  <select class="widget" style="{style}">{opts}</select>')
            else:
                html_lines.append(f'  <div class="widget" style="{style} border:1px solid #ccc; padding:4px;">{t}: {text}</div>')
        ...
        try:
            with open(path, "w", encoding="utf-8") as f:
                f.write("\n".join(html_lines))
            messagebox.showinfo("✅ Экспорт HTML", f"Файл сохранён:\n{path}")
        except Exception as e:
...
#
    def export_json(self):
        ...
        if not builder or not builder.widgets_info:
            messagebox.showwarning("Экспорт JSON", "Нет виджетов для экспорта")
            return
        ...
        export_data = {
            ...
            "widget_count": len(builder.widgets_info),
            "widgets": builder.widgets_info
        }

        try:
            with open(path, "w", encoding="utf-8") as f:
                json.dump(export_data, f, ensure_ascii=False, indent=2)
            messagebox.showinfo("✅ Экспорт JSON", f"Файл сохранён:\n{path}")
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось сохранить:\n{e}")
#
class ErrorHandler:
...
#
    ...
    def safe_execute(func, *args, **kwargs):
        ...
        try:
            ...
            print(f"[ErrorHandler] {func.__name__}: {e}\n{_tb.format_exc()}")
            return None
#
    @staticmethod
...
class EnhancedMainWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TkBuilder Ultra v9.1")
...
#
    def create_main_ui(self):
        ...
        header.pack(pady=30)
#
        tk.Label(header, text="TkBuilder Ultra v9.1",
                 font=("Segoe UI", 22, "bold"),
                 fg=ACCENT_COLOR, bg=CANVAS_BG_DARK).pack()
#
        tk.Label(header, text="Drag & Drop конструктор · Блок-логика · Шаблоны",
                 font=("Segoe UI", 11),
...
#
        ...
        super().__init__(parent)
        self.parent = parent
        self.title("TkBuilder Ultra v9.1 — Конструктор")
...
#
        ...
        self._cached_widget_props = {}          # макс 200 записей
...
#
    def launch_app(self):
        """Запускает приложение как отдельное окно — без интерфейса конструктора.
        Логика работает так же как в предпросмотре, но окно чище."""
        if not self.widgets_info:
            messagebox.showinfo("Пусто", "Сначала добавь виджеты на холст!", parent=self)
            return

        self._sync_all_props()

        # Вычисляем нужный размер окна
        max_x = max((i["x"] + i["props"].get("width",  100)) for i in self.widgets_info) + 40
        max_y = max((i["y"] + i["props"].get("height",  30)) for i in self.widgets_info) + 60
        win_w = max(300, int(max_x))
        win_h = max(200, int(max_y))

        app_win = tk.Toplevel(self)
        app_win.title("▶ Запущенное приложение")
        app_win.geometry(f"{win_w}x{win_h}")
        app_win.configure(bg="white")
        app_win.resizable(True, True)
        self.center_window_on_screen(app_win, win_w, win_h)
        ...
        for info in self.widgets_info:
            ...
            try:
                if wtype == "Button":
                    widget = tk.Button(app_win,
                        text=props.get("text","Кнопка"),
                        bg=props.get("bg","#4CAF50"), fg=props.get("fg","white"),
                        relief=props.get("relief","flat"), font=("Segoe UI",10))
                elif wtype == "Label":
                    widget = tk.Label(app_win,
                        text=props.get("text","Текст"),
                        bg=props.get("bg","#f0f0f0"), fg=props.get("fg","#333"),
                        anchor=props.get("anchor","w"), font=("Segoe UI",10))
                elif wtype == "Entry":
                    widget = tk.Entry(app_win,
                        bg=props.get("bg","white"), fg=props.get("fg","#333"),
                        font=("Segoe UI",10), relief="solid", bd=1)
                    if props.get("text"): widget.insert(0, props["text"])
                elif wtype == "Checkbutton":
                    var = tk.IntVar(value=props.get("value", 0))
                    widget = tk.Checkbutton(app_win,
                        text=props.get("text","Флажок"), variable=var,
                        bg=props.get("bg", app_win.cget("bg")),
                        fg=props.get("fg","white"),
                        selectcolor=props.get("selectcolor","#333"),
                        font=("Segoe UI",10))
                    widget._var = var; widget.var = var
                elif wtype == "Text":
                    widget = tk.Text(app_win,
                        bg=props.get("bg","white"), fg=props.get("fg","#333"),
                        font=("Segoe UI",10), relief="solid", bd=1)
                    if props.get("text"): widget.insert("1.0", props["text"])
                elif wtype == "Frame":
                    widget = tk.Frame(app_win,
                        bg=props.get("bg","#ddd"),
                        relief=props.get("relief","flat"), bd=props.get("bd",0))
                else:
                    widget = tk.Label(app_win, text=f"[{wtype}]",
                        bg="#888", fg="white", font=("Segoe UI",9))
                if widget:
                    widget.place(x=x, y=y, width=width, height=height)
                    run_widgets.append(widget)
            except Exception as e:
                print(f"launch_app [{wtype}]: {e}")

        # Применяем блок-логику
        self._apply_block_logic_to_preview(app_win, run_widgets)

    def _sync_all_props(self):
        """Синхронизирует ВСЕ props со всеми виджетами на холсте перед предпросмотром."""
        for info in self.widgets_info:
            w = info.get("widget")
            if not w:
                continue
            try:
                # размеры из canvas window
                win_id = info.get("window_id")
                if win_id:
                    try:
                        cw = self.canvas.itemcget(win_id, "width")
                        ch = self.canvas.itemcget(win_id, "height")
                        if cw: info["props"]["width"]  = int(float(cw))
                        if ch: info["props"]["height"] = int(float(ch))
                    except Exception:
                        pass
                # text, bg, fg
                for key in ("text", "bg", "fg", "font", "relief", "bd",
                            "anchor", "justify", "state", "selectcolor"):
                    try:
                        info["props"][key] = w.cget(key)
                    except Exception:
                        pass
                # Entry content
                if isinstance(w, tk.Entry):
                    info["props"]["text"] = w.get()
                elif isinstance(w, tk.Text):
                    info["props"]["text"] = w.get("1.0", "end-1c")
                # Checkbutton var
                if isinstance(w, tk.Checkbutton):
                    var = getattr(w, "_var", None) or getattr(w, "var", None)
                    if var:
                        info["props"]["value"] = var.get()
            except Exception as e:
                pass
...
#
        ...
        self._sync_all_props()  # читаем актуальные свойства с холста
...
#
                ...
                if widget_type == "Button":
                    widget = tk.Button(preview,
                                       text=props.get("text", "Кнопка"),
                                       bg=props.get("bg", "#4CAF50"),
                                       fg=props.get("fg", "white"),
                                       relief=props.get("relief", "flat"),
                                       font=("Segoe UI", 10))
#
                elif widget_type == "Label":
                    widget = tk.Label(preview,
                                      text=props.get("text", "Текст"),
...
#
                    ...
                    var = tk.IntVar(value=props.get("value", 0))
                    widget = tk.Checkbutton(preview,
                                            text=props.get("text", "Флажок"),
                                            variable=var,
                                            bg=props.get("bg", preview.cget("bg")),
                                            fg=props.get("fg", "white"),
                                            selectcolor=props.get("selectcolor", "#333"),
                                            font=("Segoe UI", 10))
                    widget.var = var
...
#
                    ...
                    widget = tk.Frame(preview,
                                      bg=props.get("bg", "#dddddd"),
                                      relief=props.get("relief", "flat"),
                                      bd=props.get("bd", 0))
#
                elif widget_type in ("LabelFrame", "Labelframe"):
                    widget = ttk.LabelFrame(preview,
                                            text=props.get("text", "Группа"),
                                            padding=5)
#
                elif widget_type == "Notebook":
                    widget = ttk.Notebook(preview)
                    for tab in props.get("tabs", ["Вкладка 1", "Вкладка 2"]):
                        widget.add(tk.Frame(widget, bg="white"), text=tab)
#
                elif widget_type in ("Separator", "HSeparator"):
                    widget = ttk.Separator(preview, orient="horizontal")
#
                elif widget_type == "VSeparator":
                    widget = ttk.Separator(preview, orient="vertical")
#
                elif widget_type == "Tooltip":
                    widget = tk.Label(preview,
                                      text=props.get("text", "💡 Подсказка"),
                                      bg="#ffffcc", fg="#333",
                                      relief="solid", bd=1,
                                      font=("Segoe UI", 9),
                                      padx=6, pady=3)
#
                    ...
                    f2 = tk.Frame(preview, bg="white", relief="solid", bd=1)
                    tk.Label(f2, text="📅", bg="white").pack(side="left", padx=4)
                    e2 = tk.Entry(f2, width=10, relief="flat", bg="white")
                    e2.insert(0, props.get("text", "дд.мм.гггг"))
                    e2.pack(side="left")
                    widget = f2
#
                elif widget_type in ("ImageLabel", "Badge", "Avatar",
                                     "Spacer", "GroupBox", "Notification"):
                    # these already handled or fallback
                    widget = tk.Label(preview,
                                      text=props.get("text", f"[{widget_type}]"),
                                      bg=props.get("bg", "#607D8B"),
                                      fg=props.get("fg", "white"),
...
#
        # БЛОК-ЛОГИКА из редактора (приоритет)
        self._apply_block_logic_to_preview(preview, preview_widgets)
        # УМНАЯ ЛОГИКА для остальных кнопок
...
#
    def _apply_block_logic_to_preview(self, preview, preview_widgets):
        """Применяет блок-логику к виджетам предпросмотра."""
        if not preview_widgets or not self.widgets_info:
            return

        # Шаг 1: строим globals со всеми виджетами и их _var
        counters = {}
        g = {"tk": tk, "messagebox": messagebox, "root": preview}
        widget_types = {}  # auto_name -> wtype

        for i, info in enumerate(self.widgets_info):
            if i >= len(preview_widgets):
                break
            wtype = info.get("type", "widget")
            base  = wtype.lower()
            counters[base] = counters.get(base, 0) + 1
            auto_name = f"{base}_{counters[base]}"
            pw = preview_widgets[i]
            g[auto_name] = pw
            widget_types[auto_name] = wtype
            stored = info.get("name", "")
            if stored:
                g[stored] = pw
                widget_types[stored] = wtype

            # Для чекбокса/радио — берём УЖЕ СУЩЕСТВУЮЩУЮ переменную
            # Никогда не создаём новую — иначе рассинхрон
            if wtype.lower() in ("checkbutton", "radiobutton"):
                var = getattr(pw, "_var", None) or getattr(pw, "var", None)
                if var is None:
                    # Последний шанс: читаем через tk internal
                    try:
                        vname = pw.cget("variable")
                        var = pw.nametowidget(str(vname)) if vname else None
                    except Exception:
                        var = None
                if var is None:
                    var = tk.IntVar()
                    try: pw.config(variable=var)
                    except Exception: pass
                pw._var = var
                pw.var  = var
                g[f"{auto_name}_var"] = var
                if stored:
                    g[f"{stored}_var"] = var

        # Шаг 2: применяем логику к каждому виджету у которого она есть
        applied = 0
        for i, info in enumerate(self.widgets_info):
            code = info.get("props", {}).get("block_logic_code", "")
            if not code or i >= len(preview_widgets):
                continue
            pw = preview_widgets[i]
            wtype = info.get("type", "")
            try:
                local_g = dict(g)
                # Ограничиваем доступные builtins для безопасности
                _safe_builtins = {
                    k: __builtins__[k] if isinstance(__builtins__, dict) else getattr(__builtins__, k)
                    for k in ("print","range","len","str","int","float","bool","list",
                              "dict","tuple","set","min","max","abs","round","sorted",
                              "enumerate","zip","map","filter","isinstance","type",
                              "hasattr","getattr","setattr","Exception")
                    if (isinstance(__builtins__, dict) and k in __builtins__)
                       or (not isinstance(__builtins__, dict) and hasattr(__builtins__, k))
                }
                local_g["__builtins__"] = _safe_builtins
                exec(code, local_g)
                # Первая функция = обработчик
                fn_names = [l.strip()[4:].split("(")[0]
                            for l in code.split("\n")
                            if l.strip().startswith("def ")]
                if not fn_names or fn_names[0] not in local_g:
                    continue
                fn = local_g[fn_names[0]]
                trigger = fn_names[0]  # on_click / on_change / on_timer

                if trigger == "on_click":
                    if wtype.lower() == "checkbutton":
                        # Чекбокс: ставим command — он вызывается ПОСЛЕ toggle
                        # Tkinter сначала меняет var, потом вызывает command
                        existing_cmd = None
                        try: existing_cmd = pw.cget("command")
                        except Exception: pass
                        def make_cb_cmd(f=fn):
                            def cmd():
                                preview.after(1, f)  # чуть позже — var уже обновлён
                            return cmd
                        pw.config(command=make_cb_cmd())
                    else:
                        try: pw.config(command=fn)
                        except Exception: pw.bind("<Button-1>", fn)
                elif trigger == "on_change":
                    pw.bind("<KeyRelease>", fn)
                elif trigger == "on_hover":
                    pw.bind("<Enter>", fn)
                elif trigger == "on_start":
                    preview.after(100, fn)
                elif trigger == "on_timer":
                    preview.after(100, fn)
                elif trigger == "on_select":
                    pw.bind("<<ListboxSelect>>", fn)
                    pw.bind("<<ComboboxSelected>>", fn)
                else:
                    try: pw.config(command=fn)
                    except Exception: pw.bind("<Button-1>", fn)
                applied += 1
            except Exception as e:
                print(f"Блок-логика preview [{wtype}]: {e}")

        if applied:
            print(f"✅ Блок-логика активна для {applied} виджет(ов) в предпросмотре")
...
#
    def setup_general_logic(self, buttons):
        """Общая логика для кнопок предпросмотра — без попапов и консольных сообщений"""
        for btn in buttons:
            text = btn.cget("text").lower()
            if not btn.cget("command"):
    ...
    def setup_advanced_general_buttons(self, preview, buttons):
        ...
        for text, btn in buttons.items():
            ...
            def make_handler(btn_text=text):
                return lambda: None  # кнопка без логики
#
...
#
    def open_app_templates(self):
        """Окно шаблонов приложений — расставляет виджеты + логику на холст"""
        win = tk.Toplevel(self)
        win.title("📋 Шаблоны приложений — TkBuilder")
        ...
        win.update_idletasks()
        sw, sh = win.winfo_screenwidth(), win.winfo_screenheight()
        win.geometry(f"860x580+{(sw-860)//2}+{(sh-580)//2}")

        # Header
        hdr = tk.Frame(win, bg="#0d2244", height=60)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        tk.Label(hdr, text="📋 Шаблоны приложений",
                 font=("Segoe UI", 16, "bold"), bg="#0d2244", fg="#00ff88").pack(
            side="left", padx=20, pady=12)
        tk.Label(hdr, text="Выбери шаблон — виджеты и логика появятся на холсте сразу",
                 font=("Segoe UI", 9), bg="#0d2244", fg="#aaa").pack(
            side="left", padx=4)

        # Scrollable grid of cards
        outer = tk.Frame(win, bg="#0f3460")
        outer.pack(fill="both", expand=True, padx=10, pady=10)
        cv = tk.Canvas(outer, bg="#0f3460", highlightthickness=0)
        sb = ttk.Scrollbar(outer, orient="vertical", command=cv.yview)
        cv.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        cv.pack(side="left", fill="both", expand=True)
        grid_frame = tk.Frame(cv, bg="#0f3460")
        cv.create_window((0, 0), window=grid_frame, anchor="nw")
        grid_frame.bind("<Configure>",
            lambda e: cv.configure(scrollregion=cv.bbox("all")))

        COLS = 2
        for i, tpl in enumerate(self.APP_TEMPLATES):
            row_i, col_i = divmod(i, COLS)
            card = tk.Frame(grid_frame, bg="#16213e", relief="ridge", bd=1,
            ...
            # Title
            tk.Label(card, text=tpl["name"], font=("Segoe UI", 12, "bold"),
                     bg="#16213e", fg="#00ff88").pack(
                anchor="w", padx=14, pady=(12, 2))
            # Description
            tk.Label(card, text=tpl["desc"], font=("Segoe UI", 9),
                     bg="#16213e", fg="#ccc", wraplength=340, justify="left").pack(
                anchor="w", padx=14)

            # Preview chips
            chips = tk.Frame(card, bg="#16213e")
            chips.pack(anchor="w", padx=14, pady=4)
            for p in tpl["preview"]:
                tk.Label(chips, text=p, font=("Segoe UI", 8),
                         bg="#0d1117", fg="#aaa", padx=6, pady=2,
            ...
            tk.Button(
                ...
                cursor="hand2",
                command=lambda t=tpl, w=win: self._apply_app_template(t, w, clear=False)
            ...
            tk.Button(
                ...
                relief="flat", padx=10, pady=4,
                cursor="hand2",
                command=lambda t=tpl, w=win: self._apply_app_template(t, w, clear=True)
            ).pack(side="left")
            # Widget count badge
            n_w = len(tpl["widgets"])
            n_l = len(tpl["logic"])
            tk.Label(btn_frame,
                     text=f"  {n_w} виджетов · {n_l} логика",
                     font=("Segoe UI", 8), bg="#16213e", fg="#555").pack(
                side="left", padx=8)

        grid_frame.columnconfigure(0, weight=1)
        grid_frame.columnconfigure(1, weight=1)

    def _apply_app_template(self, tpl, win, clear=False):
        """Расставляет виджеты шаблона на холст с логикой."""
        win.destroy()
        ...
        # --- Вычисляем смещение чтобы не налезать на существующие виджеты ---
        if self.widgets_info:
            max_x = max(i.get("x", 0) + 150 for i in self.widgets_info)
            offset_x = max_x + 40
        else:
            offset_x = 40
        offset_y = 40

        # --- Запоминаем стартовый индекс ---
        start_idx = len(self.widgets_info)

        # --- Счётчики ДО добавления (для правильных авто-имён) ---
        counters_before = {}
        for info in self.widgets_info:
            b = info.get("type", "").lower()
            counters_before[b] = counters_before.get(b, 0) + 1

        new_widgets = []   # (widget_obj, wtype, x, y, w, h)

        # --- Создаём все виджеты напрямую (без кэша) ---
        for w_def in tpl["widgets"]:
            wtype  = w_def["type"]
            props  = dict(w_def.get("props", {}))
            x      = w_def["x"] + offset_x
            y      = w_def["y"] + offset_y
            width  = w_def.get("w", 120)
            height = w_def.get("h", 32)

            try:
                # Создаём виджет напрямую на canvas
                if wtype == "Button":
                    w = tk.Button(self.canvas,
                                  text=props.get("text", "Кнопка"),
                                  bg=props.get("bg", "#4CAF50"),
                                  fg=props.get("fg", "white"),
                                  relief="flat",
                                  font=props.get("font", ("Segoe UI", 10)))
                elif wtype == "Label":
                    w = tk.Label(self.canvas,
                                 text=props.get("text", "Текст"),
                                 bg=props.get("bg", "#f0f0f0"),
                                 fg=props.get("fg", "#333"),
                                 anchor=props.get("anchor", "w"),
                    ...
                    var = tk.IntVar()
                    w = tk.Checkbutton(self.canvas,
                                       text=props.get("text", "Флажок"),
                                       bg=props.get("bg", self.canvas.cget("bg")),
                                       fg=props.get("fg", "white"),
                                       selectcolor=props.get("selectcolor", "#333"),
                                       variable=var,
                                       font=("Segoe UI", 10))
                    ...
                    w = tk.Label(self.canvas, text=f"[{wtype}]",
                                 bg="#888", fg="white")

                new_widgets.append((w, wtype, x, y, width, height))
            except Exception as e:
                print(f"Шаблон create {wtype}: {e}")

        if not new_widgets:
            messagebox.showerror("Ошибка", "Не удалось создать виджеты.", parent=self)
            return

        # --- Размещаем на холсте через create_window с явным размером ---
        # Считаем существующие виджеты ПЕРЕД добавлением
        # чтобы знать реальные номера новых виджетов на холсте
        global_counters_before = {}
        for info in self.widgets_info:
            b = info.get("type","").lower()
            global_counters_before[b] = global_counters_before.get(b, 0) + 1

        # Вычисляем реальные имена новых виджетов
        tpl_name_map = {}  # "label_1" (в шаблоне) -> "label_3" (реально на холсте)
        tmp_tpl_counters = {}
        tmp_global = dict(global_counters_before)
        for w_def in tpl["widgets"]:
            wtype_def = w_def["type"]
            base_def  = wtype_def.lower()
            tmp_tpl_counters[base_def] = tmp_tpl_counters.get(base_def, 0) + 1
            tpl_local_name   = f"{base_def}_{tmp_tpl_counters[base_def]}"
            tmp_global[base_def] = tmp_global.get(base_def, 0) + 1
            tpl_real_name    = f"{base_def}_{tmp_global[base_def]}"
            tpl_name_map[tpl_local_name] = tpl_real_name
            if base_def in ("checkbutton","radiobutton"):
                tpl_name_map[f"{tpl_local_name}_var"] = f"{tpl_real_name}_var"

        tpl_place_counters = {}
        for (w, wtype, x, y, width, height) in new_widgets:
            win_id = self.canvas.create_window(x, y, window=w,
                                               anchor="nw",
                                               width=width, height=height)
            base = wtype.lower()
            tpl_place_counters[base] = tpl_place_counters.get(base, 0) + 1
            tpl_local = f"{base}_{tpl_place_counters[base]}"
            auto_name = tpl_name_map.get(tpl_local, tpl_local)
            # Тег-лейбл
            tag_lbl = self.canvas.create_text(
                x, y - 2, anchor="sw", text=auto_name,
                font=("Consolas", 7), fill="#00ff88")
            info = {
                "type":      wtype,
                "name":      auto_name,
                "widget":    w,
            ...
            self._optimized_bind_events(w)

        # Патчим код шаблона — заменяем локальные имена на реальные
        def patch_logic_code(code):
            import re
            for local, real in sorted(tpl_name_map.items(),
                                      key=lambda kv: -len(kv[0])):
                code = re.sub(r"\b" + re.escape(local) + r"\b", real, code)
            return code

        # --- Строим exec-globals: ВСЕ виджеты с авто-именами ---
        # Используем счётчики с нуля — имена в шаблонах типа label_1, entry_1 и т.д.
        # Ключевой момент: считаем только новые виджеты шаблона
        g = {"tk": tk, "messagebox": messagebox, "root": self.winfo_toplevel()}
        tpl_counters = {}
        for info in self.widgets_info[start_idx:]:
            wtype = info.get("type", "widget")
            base  = wtype.lower()
            tpl_counters[base] = tpl_counters.get(base, 0) + 1
            aname = f"{base}_{tpl_counters[base]}"
            w_obj = info.get("widget")
            if w_obj:
                g[aname] = w_obj
            if wtype.lower() == "checkbutton":
                var = getattr(w_obj, "_var", None)
                if var:
                    g[f"{aname}_var"] = var

        # --- Привязываем логику ---
        bound = 0
        for (widget_offset, code) in tpl["logic"]:
            abs_idx = start_idx + widget_offset
            if abs_idx >= len(self.widgets_info):
                continue
            info  = self.widgets_info[abs_idx]
            w_obj = info.get("widget")
            if not w_obj:
                continue
            code = patch_logic_code(code)  # патчим имена
            info["props"]["block_logic_code"] = code
            try:
                local_g = dict(g)
                # Ограничиваем доступные builtins для безопасности
                _safe_builtins = {
                ...
                if not fn_names or fn_names[0] not in local_g:
                    continue
                fn = local_g[fn_names[0]]
                wtype_low = info.get("type", "").lower()
                if wtype_low == "checkbutton":
                    def _make(f=fn):
                        def _cmd(): self.after(1, f)
                        return _cmd
                    w_obj.config(command=_make())
                else:
                    try:    w_obj.config(command=fn)
                    except: w_obj.bind("<Button-1>", fn)
                bound += 1
            except Exception as e:
                print(f"Шаблон логика [{info.get('type')}]: {e}")

        n = len(new_widgets)
        msg = f"✅ Шаблон \"{tpl['name']}\"\n{n} виджетов добавлено на холст."
        if bound:
            msg += f"\n\n⚡ Логика привязана ({bound} виджет(ов))"
            msg += "\n▶ Нажми 'Предпросмотр' чтобы проверить"
        else:
            msg += "\n\n⚠️ Логика будет работать в Предпросмотре"
        messagebox.showinfo("Готово!", msg, parent=self)

    def open_block_logic_editor(self):
        """Открывает редактор блочной логики"""
        editor = BlockLogicEditor(self)
#
...
#
            ...
            if widget:
                ...
                self.push_history(f"Добавлен {self.selected_widget_type}")
...
#
        # Кэшируем результат (LRU eviction)
        if len(self._cached_widget_props) >= self._cache_max:
            oldest = next(iter(self._cached_widget_props))
            del self._cached_widget_props[oldest]
...
#
        ...
        categories = [
            ("🎯 ОСНОВНЫЕ", [
                ...
                ("Label",       "🏷️ Метка"),
                ("Entry",       "📝 Поле ввода"),
                ("Text",        "📄 Многостроч. текст"),
                ("Combobox",    "🔽 Выпадающий список"),
                ("Frame",       "⬜ Фрейм / Контейнер"),
                ("LabelFrame",  "🗂️ Фрейм с заголовком"),
            ]),
            ("📊 ДАННЫЕ И ВВОД", [
                ("Listbox",     "📋 Список"),
                ("Treeview",    "🌳 Дерево / Таблица"),
                ("Scale",       "🎚️ Ползунок"),
                ("Spinbox",     "🔢 Числовой счётчик"),
                ("Scrollbar",   "📜 Полоса прокрутки"),
                ("DateEntry",   "📅 Выбор даты"),
            ]),
            ("⚡ ИНТЕРАКТИВ", [
                ("Checkbutton", "☑️ Флажок"),
                ("Radiobutton", "🔘 Радиокнопка"),
                ("Progressbar", "⏳ Прогресс-бар"),
                ("Switch",      "🔛 Переключатель"),
                ("Notebook",    "📑 Вкладки (Notebook)"),
                ("Menu",        "🍔 Меню"),
            ]),
            ("🎨 СОВРЕМЕННЫЕ", [
                ("Card",         "🃏 Карточка"),
                ...
                ("ImageLabel",   "🖼️ Метка с изображением"),
            ]),
            ("📐 МАКЕТ", [
                ("HSeparator",  "─── Горизонт. линия"),
                ("VSeparator",  "│ Вертик. линия"),
                ("Spacer",      "↕️ Отступ / Spacer"),
                ("GroupBox",    "📦 Группирующий блок"),
            ]),
        ]
#
...
#
        # Категории плагинов
        self.after(100, self._add_plugin_categories)
#
        # Tools section
...
#
        ...
        action_buttons = [
            ...
            ("🧩 Блок-логика", self.open_block_logic_editor),
            ("📋 Шаблоны приложений", self.open_app_templates),
            ("▶ Запустить приложение", self.launch_app),
...
#
                ...
                return ttk.LabelFrame(self.canvas,
                                      text=p.get("text", "Группа"),
                                      padding=5)
#
            elif wtype == "Scrollbar":
                return ttk.Scrollbar(self.canvas, orient=p.get("orient", "vertical"))
#
            elif wtype == "Notebook":
                nb = ttk.Notebook(self.canvas)
                for tab in p.get("tabs", ["Вкладка 1", "Вкладка 2"]):
                    nb.add(tk.Frame(nb, bg="white"), text=tab)
                return nb
#
            elif wtype in ("Separator", "HSeparator"):
                return ttk.Separator(self.canvas, orient="horizontal")
#
            elif wtype == "VSeparator":
                return ttk.Separator(self.canvas, orient="vertical")
#
            elif wtype == "Spacer":
                return tk.Frame(self.canvas,
                                bg=p.get("bg", self.canvas.cget("bg")),
                                width=p.get("width", 20),
                                height=p.get("height", 20))
#
            elif wtype == "Tooltip":
                return tk.Label(self.canvas,
                                text=p.get("text", "💡 Подсказка"),
                                bg="#ffffcc", fg="#333",
                                relief="solid", bd=1,
                                font=("Segoe UI", 9),
                                padx=6, pady=3)
#
            elif wtype == "DateEntry":
                f = tk.Frame(self.canvas, bg="white", relief="solid", bd=1)
                tk.Label(f, text="📅", bg="white", font=("Segoe UI", 10)).pack(side="left", padx=4)
                e = tk.Entry(f, width=10, relief="flat", bg="white",
                             font=("Segoe UI", 10))
                e.insert(0, "дд.мм.гггг")
                e.pack(side="left")
                return f
#
            elif wtype == "ImageLabel":
                return tk.Label(self.canvas,
                                text=p.get("text", "🖼️ Изображение"),
                                bg=p.get("bg", "#607D8B"), fg="white",
                                font=("Segoe UI", 10),
...
#
                ...
                tk.Label(f, text=p.get("text", "Группа"),
                         bg=p.get("bg", "#f5f5f5"),
                         fg="#555",
                         font=("Segoe UI", 9, "bold")).place(x=8, y=-7)
                f.pack_propagate(False)
                return f
#
            else:
                # Fallback для неизвестных типов — показываем заглушку
                return tk.Label(self.canvas,
                                text=f"[{wtype}]",
                                bg="#607D8B", fg="white",
                                font=("Segoe UI", 9),
...
#
            ...
            print(f"create_widget_instance [{wtype}]: {e}")
            return tk.Label(self.canvas, text=f"[{wtype}]",
                            bg="#f44336", fg="white",
...
#
        if actual_widget not in self.selected_widgets:
            ...
            self._highlight_selection()
#
        # drag_data всегда инициализирован в __init__,
...
#
        ...
        self._create_treeview_settings()
        self._create_notebook_settings()
...
#
    def _create_treeview_settings(self):
        """Настройки для Treeview"""
        frame = ttk.Frame(self.specific_settings_frame)
        tk.Label(frame, text="Колонки (через запятую):",
                 bg=CANVAS_BG_DARK, fg="white").pack(anchor="w")
        cols_var = tk.StringVar(value="Название,Значение")
        ttk.Entry(frame, textvariable=cols_var).pack(fill="x", pady=3)
        tk.Label(frame, text="Строки (по одной на строку):",
                 bg=CANVAS_BG_DARK, fg="white").pack(anchor="w")
        rows_text = tk.Text(frame, height=4, width=20,
                            bg="#1a1a2e", fg="white", insertbackground="white")
        rows_text.insert("1.0", "Строка 1,Знач 1\nСтрока 2,Знач 2")
        rows_text.pack(fill="x", pady=3)
        show_var = tk.StringVar(value="headings")
        tk.Label(frame, text="Показывать:", bg=CANVAS_BG_DARK, fg="white").pack(anchor="w")
        ttk.Combobox(frame, textvariable=show_var,
                     values=["headings","tree","tree headings"]).pack(fill="x", pady=3)
        def apply():
            if not self.selected_widgets: return
            for w in self.selected_widgets:
                info = self._find_info_by_widget(w)
                if not info or info["type"] != "Treeview": continue
                try:
                    cols = [c.strip() for c in cols_var.get().split(",") if c.strip()]
                    w.config(columns=cols, show=show_var.get())
                    for c in cols:
                        w.heading(c, text=c)
                        w.column(c, width=100)
                    for item in w.get_children(): w.delete(item)
                    for line in rows_text.get("1.0","end").strip().split("\n"):
                        vals = [v.strip() for v in line.split(",")]
                        w.insert("", "end", text=vals[0] if vals else "",
                                 values=vals[1:] if len(vals)>1 else [])
                    info["props"]["columns"] = cols
                    info["props"]["show"] = show_var.get()
                except Exception as e: print(f"Treeview apply: {e}")
        ttk.Button(frame, text="Применить", command=apply).pack(fill="x", pady=5)
        self.specific_frames["Treeview"] = frame

    def _create_notebook_settings(self):
        """Настройки для Notebook (вкладки)"""
        frame = ttk.Frame(self.specific_settings_frame)
        tk.Label(frame, text="Вкладки (каждая на новой строке):",
                 bg=CANVAS_BG_DARK, fg="white").pack(anchor="w")
        tabs_text = tk.Text(frame, height=4, width=20,
                            bg="#1a1a2e", fg="white", insertbackground="white")
        tabs_text.insert("1.0", "Вкладка 1\nВкладка 2\nВкладка 3")
        tabs_text.pack(fill="x", pady=3)
        def apply():
            if not self.selected_widgets: return
            for w in self.selected_widgets:
                info = self._find_info_by_widget(w)
                if not info or info["type"] != "Notebook": continue
                try:
                    tabs = [t.strip() for t in tabs_text.get("1.0","end").strip().split("\n") if t.strip()]
                    for tab in w.tabs(): w.forget(tab)
                    for t in tabs:
                        w.add(tk.Frame(w, bg="white"), text=t)
                    info["props"]["tabs"] = tabs
                except Exception as e: print(f"Notebook apply: {e}")
        ttk.Button(frame, text="Применить", command=apply).pack(fill="x", pady=5)
        self.specific_frames["Notebook"] = frame
...
#
    def _get_widget_text(self, widget, info):
        ...
        try:
            ...
            return info["props"].get("text", "")
#
    def _clear_props_display(self):
        """Очистка панели свойств при снятии выделения"""
        try:
            self.info_label.config(
    ...
    def _update_additional_sections(self, info):
        """Обновление дополнительных секций панели свойств"""
        try:
            # Обновляем инспектор виджетов если он видим
            if hasattr(self, 'widget_inspector') and self.widget_inspector:
                self.widget_inspector.update_info(info)
        except Exception:
            pass
        try:
            # Обновляем монитор производительности
            if hasattr(self, 'perf_monitor') and self.perf_monitor:
                self.perf_monitor.update_stats(len(self.widgets_info))
        except Exception:
            pass
...
#
    def change_layer_order(self, layer_id, direction):
        """Изменяет порядок слоёв: direction = +1 вверх, -1 вниз."""
        idx = layer_id
        new_idx = idx + direction
        if new_idx < 0 or new_idx >= len(self.layers):
            return
        self.layers[idx], self.layers[new_idx] = self.layers[new_idx], self.layers[idx]
        self.layer_widgets[idx], self.layer_widgets[new_idx] = \
            self.layer_widgets[new_idx], self.layer_widgets[idx]
        # Переставляем canvas windows по z-order
        for info in self.layer_widgets.get(new_idx, []):
            wid = info.get("window_id")
            if wid:
                self.canvas.tag_raise(wid)
        if hasattr(self, "update_layers_list"):
            self.update_layers_list()
#
    def create_layer_group(self, layer_ids, group_name):
        """Создаёт именованную группу слоёв для массовых операций."""
        if not hasattr(self, "layer_groups"):
            self.layer_groups = {}
        gid = len(self.layer_groups)
        self.layer_groups[gid] = {
            "name":     group_name,
...
#
    ...
    def push_history(self, action_desc=""):
        ...
        for info in self.widgets_info:
            state.append({
                ...
                "x":         info.get("x", 0),
                "y":         info.get("y", 0),
                "props":     copy.deepcopy(info.get("props", {})),
                "layer":     info.get("layer", 0),
            })
        # Обрезаем будущее если были undo
        if self.history_index < len(self.history) - 1:
            self.history = self.history[:self.history_index + 1]
        self.history.append({"desc": action_desc, "state": state})
    ...
    def undo(self, event=None):
        ...
        self._restore_history_state(self.history[self.history_index]["state"])
    ...
    def _restore_history_state(self, state):
        """Восстанавливает состояние холста из снапшота истории."""
        # Удаляем все текущие виджеты
        for info in list(self.widgets_info):
            try:
                self.canvas.delete(info.get("window_id"))
                info.get("widget") and info["widget"].destroy()
                if info.get("tag_label"):
                    self.canvas.delete(info["tag_label"])
            except Exception:
                pass
        self.widgets_info.clear()
        self._cached_widget_props.clear()

        # Восстанавливаем из снапшота
        for snap in state:
            try:
                widget = self.create_widget_instance(snap["type"], snap["props"])
                if widget:
                    x, y = snap["x"], snap["y"]
                    w = snap["props"].get("width", 120)
                    h = snap["props"].get("height", 32)
                    win_id = self.canvas.create_window(x, y, window=widget,
                                                       anchor="nw", width=w, height=h)
                    info = {
                        "type": snap["type"], "name": snap.get("name",""),
                        "widget": widget, "window_id": win_id,
                        "x": x, "y": y,
                        "layer": snap.get("layer", 0),
                        "props": snap["props"],
                    }
                    self.widgets_info.append(info)
                    self._optimized_bind_events(widget)
            except Exception as e:
                print(f"Undo restore [{snap.get('type')}]: {e}")
...
#
    def delete_selected_widget(self):
        # Очищаем кэш удалённых виджетов
        for w in list(self.selected_widgets):
            key = f"props_{id(w)}"
            self._cached_widget_props.pop(key, None)
        if not self.selected_widgets:
            return
#
...
#
    def clear_canvas(self, confirm=True):
        if not self.widgets_info:
            return
#
        if confirm and not messagebox.askyesno("Очистка холста", "Очистить холст?"):
            return
#
        for info in list(self.widgets_info):
            try:
            ...
            for func in ai_logic_functions:
                for line in func.split('\n'):
                    if line.strip():
                        lines.append("        " + line)
#
        # Вставляем блочную логику если есть
        block_lines = []
        for info in self.widgets_info:
            p = info.get("props", {})
            if "block_logic_code" in p:
                block_lines.append("")
                block_lines.append(f"    # --- Блок-логика для {info.get('type','?')} ({info.get('name','?')}) ---")
                for bl in p["block_logic_code"].split("\n"):
                    block_lines.append("    " + bl)
        if block_lines:
            lines.append("")
            lines.append("    def setup_block_logic(self):")
            lines.extend(block_lines)
            lines.append("")
            # Also call it from __init__
            init_idx = next((i for i,l in enumerate(lines) if "self.create_widgets()" in l), None)
            if init_idx:
                lines.insert(init_idx+1, "        self.setup_block_logic()")
...
#
        extra = {
            "color_change":  self._generate_color_logic,
            "text_update":   self._generate_text_update_logic,
            "clear_field":   self._generate_clear_logic,
            "toggle":        self._generate_toggle_logic,
        }
        all_templates = {**self.logic_templates, **extra}
        if logic_type in all_templates:
            return all_templates[logic_type](widget_type, description, context)
        else:
...
#
    def _analyze_description(self, description):
        ...
        d = description.lower()
        rules = [
            ("open_window",    ["открыть окно","новое окно","toplevel","форму","диалог"]),
            ("show_message",   ["сообщение","уведомление","messagebox","показать текст","alert"]),
            ("calculate",      ["посчитать","вычислить","калькулятор","сумма","разность","плюс","минус","умножить"]),
            ("validate",       ["проверить","валидация","проверка","пустое","заполнен","email","телефон"]),
            ("navigate",       ["перейти","навигация","страница","вкладку","tab"]),
            ("data_operation", ["сохранить","загрузить","файл","записать","читать","json","csv"]),
            ("color_change",   ["цвет","покрасить","сменить цвет","фон","bg"]),
            ("text_update",    ["изменить текст","поменять текст","обновить метку","label"]),
            ("clear_field",    ["очистить","стереть","сбросить","clear"]),
            ("toggle",         ["включить","выключить","вкл","выкл","переключить","toggle"]),
        ]
        for logic_type, keywords in rules:
...
"""
        return logic_code
#
    def _generate_color_logic(self, wt, desc, ctx):
        import re
        colors = re.findall(r"#[0-9a-fA-F]{3,6}|красн|зелен|синий|жёлт", desc)
        color = colors[0] if colors else "#00aaff"
        color_map = {"красн": "#ff4444", "зелен": "#44bb44",
                     "синий": "#4444ff", "жёлт": "#ffcc00"}
        color = color_map.get(color, color)
        return f"def on_click(event=None):\n    current_widget.config(bg=\"{color}\")"   
#
    def _generate_text_update_logic(self, wt, desc, ctx):
        import re
        quoted = re.findall(r'["\']([^"\']+)["\']', desc)
        text = quoted[0] if quoted else "Новый текст"
        return f"def on_click(event=None):\n    label_1.config(text=\"{text}\")"   
#
    def _generate_clear_logic(self, wt, desc, ctx):
        return ("def on_click(event=None):\n"
                "    try: entry_1.delete(0, 'end')\n"
                "    except: pass")
#
    def _generate_toggle_logic(self, wt, desc, ctx):
        return ("def on_click(event=None):\n"
                "    cur = current_widget.cget('text')\n"
                "    current_widget.config(text='Выкл' if cur=='Вкл' else 'Вкл')")
#
...
# ================================================================
# БЛОЧНОЕ ПРОГРАММИРОВАНИЕ — BlockLogicEditor
# Claude (Anthropic) v9.1
# ================================================================

BLOCK_TRIGGERS = [
    ("🖱️ Нажата кнопка",          "on_click"),
    ("✏️ Текст изменён",           "on_change"),
    ("📋 Элемент выбран",          "on_select"),
    ("🚀 При запуске",             "on_start"),
    ("⏱️ По таймеру",              "on_timer"),
    ("🖱️ Наведение мыши",          "on_hover"),
]
BLOCK_CONDITIONS = [
    ("📝 Если текст равен",        "if_text_eq"),
    ("🔢 Если число больше",       "if_num_gt"),
    ("🔢 Если число меньше",       "if_num_lt"),
    ("☑️ Если флажок установлен",  "if_checked"),
    ("📋 Если поле не пусто",      "if_not_empty"),
]
BLOCK_ACTIONS = [
    ("💬 Показать сообщение",      "show_msg"),
    ("📝 Изменить текст виджета",  "set_text"),
    ("🎨 Изменить цвет виджета",   "set_color"),
    ("🪟 Открыть новое окно",      "open_window"),
    ("🗑️ Очистить поле",           "clear_field"),
    ("📋 Скопировать в буфер",     "copy_clip"),
    ("💾 Сохранить в файл",        "save_file"),
    ("📂 Открыть файл",            "open_file"),
    ("❌ Закрыть окно",            "close_window"),
    ("🔁 Повторить N раз",         "repeat_n"),
]
BLOCK_COLORS = {
    "trigger":   "#1565C0",
    "condition": "#6A1B9A",
...
BLOCK_DEFAULT_PARAMS = {
    "show_msg":    {"title": "Сообщение", "text": "Привет!"},
    "set_text":    {"target": "__widget__", "text": "Новый текст"},
    "set_color":   {"target": "__widget__", "color": "#00ff88"},
    "open_window": {"title": "Новое окно", "width": "400", "height": "300"},
    "clear_field": {"target": "__widget__"},
    "copy_clip":   {"source": "__widget__"},
    "save_file":   {"source": "__widget__", "filename": "output.txt"},
    "on_timer":    {"interval_ms": "1000"},
    "if_text_eq":  {"source": "__widget__", "value": ""},
    "if_num_gt":   {"source": "__widget__", "number": "0"},
    "if_num_lt":   {"source": "__widget__", "number": "0"},
    "repeat_n":    {"count": "3"},
}
# Какие параметры — выпадающий список виджетов
WIDGET_SELECT_PARAMS = {"target", "source"}


class BlockLogicEditor(tk.Toplevel):
    """Редактор блочной логики v9.1"""

    def __init__(self, builder):
        super().__init__(builder)
        self.builder = builder
        self.title("🧩 Блочное программирование — TkBuilder v9.1")
        self.geometry("1150x700")
        self.configure(bg="#1a1a2e")
        self.resizable(True, True)
        self.protocol("WM_DELETE_WINDOW", self.destroy)

        self.program_blocks = []      # [{label, code, kind, params}]
        self.selected_idx = None      # индекс выбранного блока
        self._prop_vars = {}          # StringVar для полей свойств
        self._prop_widgets = {}       # сами виджеты полей свойств
        self._banner = None
        self._code_preview_widget = None

        self._build_ui()
        self._populate_widget_list()
        # после создания combo — привязываем смену виджета
        self.widget_combo.bind("<<ComboboxSelected>>", self._on_target_widget_changed)
        self._center()
    ...
    def _build_ui(self):
        # ---- топ-бар ----
        top = tk.Frame(self, bg="#0f3460", height=50)
        top.pack(fill="x")
        top.pack_propagate(False)
        tk.Label(top, text="🧩 БЛОЧНОЕ ПРОГРАММИРОВАНИЕ",
                 font=("Segoe UI", 14, "bold"),
                 bg="#0f3460", fg="#00ff88").pack(side="left", padx=16)
        tk.Label(top, text="💡 Целевой виджет = тот, на который нажимают для запуска логики",
                 font=("Segoe UI", 8), bg="#0f3460", fg="#aaa").pack(side="left", padx=8)
        for txt, cmd, bg_c in [
            ("✅ Применить",   self._apply_to_widget,  "#00c853"),
            ("📋 Код",         self._preview_code,     "#0088ff"),
            ("📚 Шаблоны",     self._open_templates,   "#7b1fa2"),
            ("🗑️ Очистить",    self._clear_all,        "#c62828"),
        ]:
            tk.Button(top, text=txt, font=("Segoe UI", 10, "bold"),
                      bg=bg_c, fg="white" if bg_c != "#00c853" else "black",
                      relief="flat", padx=12, pady=4,
                      command=cmd).pack(side="right", padx=6, pady=8)
        ...
        self._build_program_area(body)
        self._build_right_panel(body)

    # ── левая палитра блоков ───────────────────────────────────
    def _build_palette(self, parent):
        frame = tk.Frame(parent, bg="#16213e", width=230)
        frame.pack(side="left", fill="y", padx=(4, 2), pady=4)
        frame.pack_propagate(False)

        tk.Label(frame, text="📦 Блоки", font=("Segoe UI", 11, "bold"),
                 bg="#16213e", fg="#00ff88").pack(pady=(10, 4), padx=10, anchor="w")

        # прокрутка внутри палитры
        c = tk.Canvas(frame, bg="#16213e", highlightthickness=0)
        sb = ttk.Scrollbar(frame, orient="vertical", command=c.yview)
        c.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        c.pack(side="left", fill="both", expand=True)
        inner = tk.Frame(c, bg="#16213e")
        ...
        def section(title, items, kind):
            ...
            hdr = tk.Frame(inner, bg=color, height=24)
            ...
            for lbl, code in items:
                b = tk.Button(inner, text=lbl, font=("Segoe UI", 9),
                              bg="#1e2d4a", fg="white", relief="flat",
                              anchor="w", padx=8, cursor="hand2",
                              activebackground=color, activeforeground="white",
                              command=lambda l=lbl, c=code, k=kind: self._add_block(l, c, k))
                b.pack(fill="x", padx=4, pady=1)

        section("⚡ ТРИГГЕРЫ",  BLOCK_TRIGGERS,   "trigger")
        section("❓ УСЛОВИЯ",   BLOCK_CONDITIONS, "condition")
        section("⚙️ ДЕЙСТВИЯ",  BLOCK_ACTIONS,    "action")

    # ── центральная область программы ─────────────────────────
    def _build_program_area(self, parent):
        frame = tk.Frame(parent, bg="#1a1a2e")
        frame.pack(side="left", fill="both", expand=True, padx=2, pady=4)

        tk.Label(frame, text="📋 Программа", font=("Segoe UI", 10, "bold"),
                 bg="#1a1a2e", fg="#aaa").pack(pady=(6, 2), padx=10, anchor="w")

        wrap = tk.Frame(frame, bg="#1a1a2e")
        wrap.pack(fill="both", expand=True, padx=8, pady=(0, 4))

        self.prog_canvas = tk.Canvas(wrap, bg="#0d1117",
                                     highlightthickness=1,
                                     highlightbackground="#00ff88")
        vsb = ttk.Scrollbar(wrap, orient="vertical", command=self.prog_canvas.yview)
        self.prog_canvas.configure(yscrollcommand=vsb.set)
        vsb.pack(side="right", fill="y")
        self.prog_canvas.pack(side="left", fill="both", expand=True)

        self.prog_frame = tk.Frame(self.prog_canvas, bg="#0d1117")
        self._prog_win_id = self.prog_canvas.create_window(
            (0, 0), window=self.prog_frame, anchor="nw")
        self.prog_frame.bind("<Configure>",
            lambda e: self.prog_canvas.configure(scrollregion=self.prog_canvas.bbox("all")))
        self.prog_canvas.bind("<Configure>",
            lambda e: self.prog_canvas.itemconfig(self._prog_win_id, width=e.width))

        self._draw_hint()

    # ── правая панель: виджет + свойства блока + порядок ──────
    def _build_right_panel(self, parent):
        frame = tk.Frame(parent, bg="#16213e", width=280)
        frame.pack(side="right", fill="y", padx=(2, 4), pady=4)
        frame.pack_propagate(False)

        # -- выбор целевого виджета --
        tk.Label(frame, text="🎯 Целевой виджет",
                 font=("Segoe UI", 10, "bold"), bg="#16213e", fg="#00ff88").pack(
            pady=(12, 4), padx=10, anchor="w")
        self.widget_var = tk.StringVar()
        self.widget_combo = ttk.Combobox(frame, textvariable=self.widget_var,
                                          state="readonly", width=30)
        self.widget_combo.pack(padx=10, pady=(0, 4), fill="x")

        ttk.Separator(frame, orient="horizontal").pack(fill="x", padx=10, pady=8)

        # -- свойства выбранного блока --
        tk.Label(frame, text="⚙️ Свойства блока",
                 font=("Segoe UI", 10, "bold"), bg="#16213e", fg="#00ff88").pack(
            pady=(0, 4), padx=10, anchor="w")

        # контейнер со скроллом для свойств
        props_outer = tk.Frame(frame, bg="#16213e", height=220)
        props_outer.pack(fill="x", padx=6, pady=(0, 4))
        props_outer.pack_propagate(False)
        self.props_frame = tk.Frame(props_outer, bg="#16213e")
        self.props_frame.pack(fill="both", expand=True)

        ttk.Separator(frame, orient="horizontal").pack(fill="x", padx=10, pady=8)

        # -- порядок блоков --
        tk.Label(frame, text="🔀 Порядок / удаление",
                 font=("Segoe UI", 10, "bold"), bg="#16213e", fg="#00ff88").pack(
            pady=(0, 4), padx=10, anchor="w")
        ctrl = tk.Frame(frame, bg="#16213e")
        ctrl.pack(fill="x", padx=10)
        for txt, cmd, bg_c in [
            ("⬆ Вверх",   self._move_up,          "#1e2d4a"),
            ("⬇ Вниз",    self._move_down,         "#1e2d4a"),
            ("🗑 Удалить", self._delete_selected,   "#c62828"),
        ]:
            tk.Button(ctrl, text=txt, font=("Segoe UI", 9),
    ...
    def _populate_widget_list(self):
        names = []
        for info in self.builder.widgets_info:
            t = info.get("type", "?")
            n = info.get("name", "?")
            names.append(f"{t} — {n}")
        self.widget_combo["values"] = names or ["— нет виджетов —"]
        if names:
            self.widget_combo.current(0)

    def _get_widget_names(self):
        """Возвращает список (отображаемое_имя, код_имя) для каждого виджета."""
        result = []
        for info in self.builder.widgets_info:
            t    = info.get("type", "Widget")
            name = info.get("name", "widget")
            # Отображаем: "Button — button_1", в коде используется "button_1"
            result.append((f"{t}  —  {name}", name))
        return result

    # ── смена целевого виджета ─────────────────────────────────
    def _on_target_widget_changed(self, event=None):
        idx = self.widget_combo.current()
        if idx < 0 or idx >= len(self.builder.widgets_info):
            return
        info = self.builder.widgets_info[idx]
        existing = info.get("props", {}).get("block_logic_code", "")
        ...
        self._hide_banner()
        if existing:
            self._show_existing_banner(existing)

    # ── баннеры существующей логики ───────────────────────────
    def _show_existing_banner(self, code):
        self._hide_banner()
        self._banner = tk.Frame(self.prog_frame, bg="#1B5E20", relief="ridge", bd=1)
        self._banner.pack(fill="x", padx=10, pady=6)
        tk.Label(self._banner, text="✅ У этого виджета уже есть блок-логика",
                 font=("Segoe UI", 9, "bold"), bg="#1B5E20", fg="#00ff88").pack(
            side="left", padx=10, pady=6)
        tk.Button(self._banner, text="Переписать",
                  font=("Segoe UI", 8), bg="#c62828", fg="white", relief="flat",
                  command=self._reset_widget_logic).pack(side="right", padx=8, pady=4)
        self._code_preview_widget = tk.Text(
            self.prog_frame, height=7, font=("Consolas", 8),
            bg="#0d2b0d", fg="#88ff88", state="normal", wrap="none")
        self._code_preview_widget.insert("1.0", code)
        self._code_preview_widget.config(state="disabled")
        self._code_preview_widget.pack(fill="x", padx=10, pady=(0, 4))

    def _hide_banner(self):
        for attr in ("_banner", "_code_preview_widget"):
    ...
    def _reset_widget_logic(self):
        idx = self.widget_combo.current()
        if 0 <= idx < len(self.builder.widgets_info):
            self.builder.widgets_info[idx]["props"].pop("block_logic_code", None)
        self._hide_banner()
        self.program_blocks.clear()
        self.selected_idx = None
        self._clear_props_panel()
    ...
    # ── добавление блока ───────────────────────────────────────
    def _add_block(self, label, code, kind):
        import copy
        params = copy.deepcopy(BLOCK_DEFAULT_PARAMS.get(code, {}))
        all_widgets = self._get_widget_names()  # [(display, name), ...]
        first_any   = all_widgets[0][1] if all_widgets else ""
        # Для if_checked предпочитаем найти Checkbutton
        cb_name = next(
            (name for disp, name in all_widgets
             if "checkbutton" in disp.lower() or "check" in disp.lower()),
            first_any
        )
        for k in params:
            if params[k] == "__widget__":
                params[k] = cb_name if code == "if_checked" else first_any
        block = {"label": label, "code": code, "kind": kind, "params": params}
        self.program_blocks.append(block)
        new_idx = len(self.program_blocks) - 1
        self.selected_idx = new_idx
        self._redraw_program()
        self._show_props(new_idx)

    # ── перерисовка списка блоков ──────────────────────────────
    def _draw_hint(self):
        lbl = tk.Label(self.prog_frame,
                       text="← Нажми на блок слева, чтобы добавить его сюда",
                       font=("Segoe UI", 11), bg="#0d1117", fg="#555")
        lbl.pack(pady=50)

    def _redraw_program(self):
        # Уничтожаем только блоки, не баннер
        children = list(self.prog_frame.winfo_children())
        banner_widgets = set()
        if self._banner:
            banner_widgets.add(self._banner)
        if self._code_preview_widget:
            banner_widgets.add(self._code_preview_widget)
        for w in children:
            if w not in banner_widgets:
                w.destroy()

        if not self.program_blocks:
            self._draw_hint()
            return

        for i, block in enumerate(self.program_blocks):
            color = BLOCK_COLORS[block["kind"]]
            is_sel = (i == self.selected_idx)

            row = tk.Frame(self.prog_frame, bg="#0d1117")
            row.pack(fill="x", padx=10, pady=2)

            # номер
            num_lbl = tk.Label(row, text=f"{i+1:02d}", font=("Consolas", 9),
                                bg="#0d1117", fg="#555", width=3)
            num_lbl.pack(side="left")

            # карточка блока
            card_bg = color
            card_border = "#ffffff" if is_sel else color
            card = tk.Frame(row, bg=card_bg,
                            highlightthickness=2 if is_sel else 0,
                            highlightbackground="#ffffff",
                            cursor="hand2")
            card.pack(side="left", fill="x", expand=True, ipady=6, ipadx=10)

            kind_icons = {"trigger": "⚡", "condition": "❓", "action": "⚙️"}
            icon = kind_icons.get(block["kind"], "")
            # краткий показ параметров
            param_str = ""
            if block["params"]:
                parts = []
            ...
            lbl_text = f"{icon} {block['label']}{param_str}"
            font_w = "bold" if is_sel else "normal"
            lbl = tk.Label(card, text=lbl_text, font=("Segoe UI", 10, font_w),
                           bg=card_bg, fg="white", anchor="w")
            lbl.pack(side="left", fill="x", expand=True)

            # ▶ ВОТ КЛЮЧЕВОЙ ФИХ: bind и на Frame и на Label
            for widget in (card, lbl, num_lbl):
                widget.bind("<Button-1>", lambda e, idx=i: self._select_block(idx))

            # стрелка между блоками
            if i < len(self.program_blocks) - 1:
                arr = tk.Label(self.prog_frame, text="     ↓",
                               font=("Segoe UI", 8), bg="#0d1117", fg="#444")
                arr.pack(anchor="w", padx=30)
    ...
    def _select_block(self, idx):
        ...
        self._redraw_program()
        self._show_props(idx)

    # ── панель свойств ─────────────────────────────────────────
    def _clear_props_panel(self):
        for w in self.props_frame.winfo_children():
            w.destroy()
        self._prop_vars.clear()
        self._prop_widgets.clear()

    def _show_props(self, idx):
        self._clear_props_panel()
        if idx is None or idx >= len(self.program_blocks):
            return

        block = self.program_blocks[idx]
        bg = "#16213e"

        tk.Label(self.props_frame, text=block["label"],
                 font=("Segoe UI", 9, "bold"), bg=bg, fg="#00ff88").pack(
            anchor="w", pady=(0, 6))

        if not block["params"]:
            tk.Label(self.props_frame, text="Нет параметров",
                     bg=bg, fg="#555", font=("Segoe UI", 8)).pack()
            return

        # widget_entries: list of (display, code_name)
        widget_entries = self._get_widget_names()
        display_labels = [d for d, _ in widget_entries]
        code_names     = [c for _, c in widget_entries]

        for key, val in block["params"].items():
            row = tk.Frame(self.props_frame, bg=bg)
            row.pack(fill="x", pady=3)
            tk.Label(row, text=key + ":", bg=bg, fg="#aaa",
                     font=("Segoe UI", 8), width=10, anchor="w").pack(side="left")

            var = tk.StringVar(value=str(val))
            self._prop_vars[key] = var

            # ▶ target/source — Combobox с понятными именами
            if key in WIDGET_SELECT_PARAMS and widget_entries:
                # Найти текущий display по code_name
                cur_display = ""
                if val in code_names:
                    cur_display = display_labels[code_names.index(val)]
                elif display_labels:
                    cur_display = display_labels[0]
                    var.set(code_names[0])
                    # сохраняем code_name сразу
                    if 0 <= idx < len(self.program_blocks):
                        self.program_blocks[idx]["params"][key] = code_names[0]

                disp_var = tk.StringVar(value=cur_display)

                cb = ttk.Combobox(row, textvariable=disp_var,
                                  values=display_labels, state="readonly", width=20)
                cb.pack(side="left", fill="x", expand=True)
                self._prop_widgets[key] = cb

                # Подсказка: "в коде: button_1"
                hint_var = tk.StringVar(value=f"→ {var.get()}")
                hint_lbl = tk.Label(self.props_frame, textvariable=hint_var,
                                    bg=bg, fg="#00ff88",
                                    font=("Consolas", 8))
                hint_lbl.pack(anchor="w", padx=12, pady=(0, 2))

                def on_widget_select(e, k=key, dv=disp_var, cv=var,
                                     cn=code_names, dl=display_labels,
                                     i=idx, hv=hint_var):
                    chosen_display = dv.get()
                    if chosen_display in dl:
                        code = cn[dl.index(chosen_display)]
                        cv.set(code)
                        hv.set(f"→ {code}")
                        self._update_param_silent(i, k, code)

                cb.bind("<<ComboboxSelected>>", on_widget_select)
            else:
                ent = tk.Entry(row, textvariable=var, font=("Segoe UI", 9),
                               bg="#0d1117", fg="white", insertbackground="white",
                               relief="flat", width=16)
                ent.pack(side="left", fill="x", expand=True)
                self._prop_widgets[key] = ent
                ent.bind("<FocusOut>",
                         lambda e, k=key, v=var, i=idx: self._update_param_silent(i, k, v.get()))
                ent.bind("<Return>",
                         lambda e, k=key, v=var, i=idx: self._update_param_silent(i, k, v.get()))

    def _update_param_silent(self, idx, key, value):
        """Сохраняет параметр БЕЗ перерисовки — курсор не сбрасывается"""
        if 0 <= idx < len(self.program_blocks):
            self.program_blocks[idx]["params"][key] = value

    # ── перемещение и удаление ─────────────────────────────────
    def _move_up(self):
        i = self.selected_idx
        if i is not None and i > 0:
            self.program_blocks[i], self.program_blocks[i-1] = \
                self.program_blocks[i-1], self.program_blocks[i]
            self.selected_idx = i - 1
            self._redraw_program()
            self._show_props(self.selected_idx)

    def _move_down(self):
        i = self.selected_idx
        if i is not None and i < len(self.program_blocks) - 1:
            self.program_blocks[i], self.program_blocks[i+1] = \
                self.program_blocks[i+1], self.program_blocks[i]
            self.selected_idx = i + 1
            self._redraw_program()
    ...
    def _delete_selected(self):
        i = self.selected_idx
        if i is not None and 0 <= i < len(self.program_blocks):
            self.program_blocks.pop(i)
            self.selected_idx = None
            self._clear_props_panel()
            self._redraw_program()

    def _clear_all(self):
        if messagebox.askyesno("Очистить", "Удалить все блоки?", parent=self):
            self.program_blocks.clear()
            self.selected_idx = None
            self._clear_props_panel()
            self._redraw_program()
    ...
    TEMPLATES = [
        {
            "name": "💬 Кнопка → сообщение",
            "desc": "Нажатие кнопки показывает всплывающее сообщение.",
            "who":  "Любая кнопка (Button)",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click",  "trigger",   {}),
                ("💬 Показать сообщение",   "show_msg",  "action",    {"title": "Привет!", "text": "Кнопка нажата!"}),
            ]
        },
        {
            "name": "✏️ Поле → метка",
            "desc": "Когда меняется текст в поле ввода — метка обновляется.",
            "who":  "Entry (поле ввода) как триггер, Label как target",
            "blocks": [
                ("✏️ Текст изменён",         "on_change", "trigger",   {}),
                ("📝 Изменить текст виджета","set_text",  "action",    {"target": "__first__", "text": ""}),
            ]
        },
        {
            "name": "☑️ Флажок → действие",
            "desc": "Если флажок установлен — меняет цвет виджета.",
            "who":  "Checkbutton как source, любой виджет как target",
            "blocks": [
                ("🖱️ Нажата кнопка",        "on_click",    "trigger",   {}),
                ("☑️ Если флажок установлен","if_checked",  "condition", {"source": "__check__"}),
                ("🎨 Изменить цвет виджета", "set_color",   "action",    {"target": "__first__", "color": "#00ff88"}),
            ]
        },
        {
            "name": "🪟 Кнопка → открыть окно",
            "desc": "Нажатие кнопки открывает новое окно.",
            "who":  "Button",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click",    "trigger",  {}),
                ("🪟 Открыть новое окно",   "open_window", "action",   {"title": "Новое окно", "width": "400", "height": "300"}),
            ]
        },
        {
            "name": "📋 Не пусто → сообщение",
            "desc": "Если поле ввода не пустое — показывает его содержимое.",
            "who":  "Button + Entry",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click",     "trigger",   {}),
                ("📋 Если поле не пусто",   "if_not_empty", "condition", {"source": "__entry__"}),
                ("💬 Показать сообщение",   "show_msg",     "action",    {"title": "Данные", "text": "Поле заполнено!"}),
            ]
        },
        {
            "name": "🗑️ Кнопка → очистить поле",
            "desc": "Нажатие кнопки очищает поле ввода.",
            "who":  "Button + Entry",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click",    "trigger",  {}),
                ("🗑️ Очистить поле",        "clear_field", "action",   {"target": "__entry__"}),
            ]
        },
        {
            "name": "⏱️ Таймер → обновить метку",
            "desc": "Каждые N мс обновляет текст метки.",
            "who":  "Label как target",
            "blocks": [
                ("⏱️ По таймеру",            "on_timer",  "trigger",  {"interval_ms": "1000"}),
                ("📝 Изменить текст виджета","set_text",  "action",   {"target": "__label__", "text": "Обновлено!"}),
            ]
        },
        {
            "name": "💾 Кнопка → сохранить в файл",
            "desc": "Сохраняет содержимое поля ввода в файл.",
            "who":  "Button + Entry",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click",  "trigger",  {}),
                ("💾 Сохранить в файл",     "save_file", "action",   {"source": "__entry__", "filename": "output.txt"}),
            ]
        },
        {
            "name": "☑️ Флажок → сразу меняет текст",
            "desc": "Ставишь/снимаешь галочку — текст метки меняется мгновенно. Применяй на Checkbutton.",
            "who":  "Checkbutton (применять на флажок!), Label как target",
            "blocks": [
                ("🖱️ Нажата кнопка",        "on_click", "trigger", {}),
                ("📝 Изменить текст виджета","set_text",  "action",  {"target": "__label__", "text": "Включено!"}),
            ]
        },
        {
            "name": "☑️ Флажок → цвет вкл/выкл",
            "desc": "Галочка → зелёный, снята → красный. Применяй на Checkbutton.",
            "who":  "Checkbutton (применять на флажок!), любой виджет как target",
            "blocks": [
                ("🖱️ Нажата кнопка",       "on_click", "trigger", {}),
                ("🎨 Изменить цвет виджета","set_color", "action",  {"target": "__first__", "color": "#00ff88"}),
            ]
    ...
    def _open_templates(self):
        """Окно выбора шаблонов блочной логики"""
        win = tk.Toplevel(self)
        win.title("📚 Шаблоны блочной логики")
        ...
        win.update_idletasks()
        sw, sh = win.winfo_screenwidth(), win.winfo_screenheight()
        win.geometry(f"680x540+{(sw-680)//2}+{(sh-540)//2}")

        tk.Label(win, text="📚 Выбери шаблон — блоки загрузятся автоматически",
                 font=("Segoe UI", 12, "bold"), bg="#0f3460", fg="#00ff88").pack(
            pady=(14, 4), padx=16, anchor="w")
        tk.Label(win, text="Шаблон заменит текущие блоки. target/source подставится автоматически.",
                 font=("Segoe UI", 9), bg="#0f3460", fg="#aaa").pack(padx=16, anchor="w")

        scroll_wrap = tk.Frame(win, bg="#0f3460")
        scroll_wrap.pack(fill="both", expand=True, padx=12, pady=10)
        cv = tk.Canvas(scroll_wrap, bg="#0f3460", highlightthickness=0)
        sb = ttk.Scrollbar(scroll_wrap, orient="vertical", command=cv.yview)
        cv.configure(yscrollcommand=sb.set)
        sb.pack(side="right", fill="y")
        cv.pack(side="left", fill="both", expand=True)
        ...
        for tpl in self.TEMPLATES:
            card = tk.Frame(inner, bg="#16213e", relief="flat", bd=0)
            card.pack(fill="x", padx=6, pady=5, ipady=8)

            top_row = tk.Frame(card, bg="#16213e")
            top_row.pack(fill="x", padx=12, pady=(8, 2))

            tk.Label(top_row, text=tpl["name"],
                     font=("Segoe UI", 11, "bold"), bg="#16213e", fg="#00ff88").pack(
                side="left")

            tk.Button(top_row, text="➕ Применить шаблон",
                      font=("Segoe UI", 9, "bold"),
                      bg="#00c853", fg="black", relief="flat", padx=10,
                      cursor="hand2",
                      command=lambda t=tpl, w=win: self._apply_template(t, w)).pack(
                side="right", padx=4)

            tk.Label(card, text=tpl["desc"],
                     font=("Segoe UI", 9), bg="#16213e", fg="white",
                     anchor="w", wraplength=450, justify="left").pack(
                fill="x", padx=12)
            tk.Label(card, text=f"🎯 Для: {tpl['who']}",
                     font=("Segoe UI", 8), bg="#16213e", fg="#aaa",
                     anchor="w").pack(fill="x", padx=12, pady=(2, 6))

            # Превью блоков
            blocks_row = tk.Frame(card, bg="#16213e")
            blocks_row.pack(fill="x", padx=12, pady=(0, 6))
            for i, (lbl, code, kind, _) in enumerate(tpl["blocks"]):
                color = BLOCK_COLORS[kind]
                b = tk.Label(blocks_row, text=lbl, font=("Segoe UI", 8),
                             bg=color, fg="white", padx=6, pady=2)
                b.pack(side="left", padx=2)
                if i < len(tpl["blocks"]) - 1:
                    tk.Label(blocks_row, text="→", font=("Segoe UI", 8),
                             bg="#16213e", fg="#555").pack(side="left")

    def _apply_template(self, tpl, win):
        """Применяет шаблон — заменяет текущие блоки"""
        import copy
        # Собираем имена виджетов для подстановки
        all_w   = self._get_widget_names()           # [(display, name)]
        first   = all_w[0][1]   if all_w else ""
        entries = [n for d, n in all_w if "entry" in d.lower()]
        labels  = [n for d, n in all_w if "label" in d.lower()]
        checks  = [n for d, n in all_w if "checkbutton" in d.lower() or "check" in d.lower()]
        first_entry = entries[0] if entries else first
        first_label = labels[0]  if labels  else first
        first_check = checks[0]  if checks  else first

        def resolve(val):
            if val == "__first__":  return first
            if val == "__entry__":  return first_entry
            if val == "__label__":  return first_label
            if val == "__check__":  return first_check
            return val

        self.program_blocks.clear()
        for lbl, code, kind, params in tpl["blocks"]:
            p = {k: resolve(v) for k, v in params.items()}
            # Также подставляем defaults если пустые
            defaults = copy.deepcopy(BLOCK_DEFAULT_PARAMS.get(code, {}))
            for k, v in defaults.items():
                if k not in p or not p[k]:
                    p[k] = resolve(v) if v == "__widget__" else v
            self.program_blocks.append({"label": lbl, "code": code, "kind": kind, "params": p})

        self.selected_idx = None
        self._clear_props_panel()
        self._redraw_program()
        win.destroy()
        # Автоматически выбираем первый блок
        if self.program_blocks:
            self._select_block(0)

    # ── генерация кода ─────────────────────────────────────────
    def _generate_code(self):
        """Генерирует Python-код из блоков.
        Имена виджетов берутся как есть — они инжектируются в globals при exec."""
        if not self.program_blocks:
            return ""
        lines = []
        indent = ""
        ...
        for block in self.program_blocks:
            ...
            # ── ТРИГГЕРЫ ──────────────────────────────────────
            if code == "on_click":
                lines.append("def on_click(event=None):")
                indent = "    "
            elif code == "on_change":
                lines.append("def on_change(event=None):")
                indent = "    "
            elif code == "on_select":
                lines.append("def on_select(event=None):")
                indent = "    "
            elif code == "on_start":
                lines.append("def on_start():")
                indent = "    "
            elif code == "on_hover":
                lines.append("def on_hover(event=None):")
                indent = "    "
            elif code == "on_timer":
                has_timer = True
                timer_ms = int(p.get("interval_ms", 1000))
                lines.append("def on_timer():")
                indent = "    "

            # ── УСЛОВИЯ ───────────────────────────────────────
            elif code == "if_text_eq":
                src = p.get("source", "") or "entry"
                val = p.get("value", "")
                lines.append(f"{indent}if {src}.get() == {repr(val)}:")
                indent += "    "
            elif code == "if_num_gt":
                src = p.get("source", "") or "entry"
                num = p.get("number", "0")
                lines.append(f"{indent}try:")
                lines.append(f"{indent}    if float({src}.get()) > {num}:")
                indent += "        "
            elif code == "if_num_lt":
                src = p.get("source", "") or "entry"
                num = p.get("number", "0")
                ...
                lines.append(f"{indent}    if float({src}.get()) < {num}:")
                indent += "        "
            elif code == "if_checked":
                # source должен быть именем Checkbutton виджета (напр. checkbutton_1)
                src = p.get("source", "").strip()
                if not src:
                    # пытаемся найти первый чекбокс автоматически
                    src = next(
                        (i.get("name","") for i in self.builder.widgets_info
                         if i.get("type","").lower() in ("checkbutton","check")),
                        ""
                    )
                if not src:
                    src = "checkbutton_1"  # крайний fallback
                lines.append(f"{indent}if {src}_var.get():")
                indent += "    "
            elif code == "if_not_empty":
                src = p.get("source", "") or "entry"
                lines.append(f"{indent}if {src}.get().strip():")
                indent += "    "

            # ── ДЕЙСТВИЯ ──────────────────────────────────────
            elif code == "show_msg":
                title = p.get("title", "Сообщение")
                text  = p.get("text", "Готово!")
                lines.append(f"{indent}messagebox.showinfo({repr(title)}, {repr(text)})")
            elif code == "set_text":
                tgt = p.get("target", "") or "label"
                txt = p.get("text", "")
                lines.append(f"{indent}{tgt}.config(text={repr(txt)})")
            elif code == "set_color":
                tgt = p.get("target", "") or "widget"
                col = p.get("color", "#ff0000")
                lines.append(f"{indent}{tgt}.config(bg={repr(col)})")
            elif code == "open_window":
                w = p.get("width", "400")
                h = p.get("height", "300")
                title = p.get("title", "Окно")
                lines.append(f"{indent}_win = tk.Toplevel()")
                lines.append(f"{indent}_win.title({repr(title)})")
                lines.append(f"{indent}_win.geometry('{w}x{h}')")
            elif code == "clear_field":
                tgt = p.get("target", "") or "entry"
                lines.append(f"{indent}try: {tgt}.delete(0, 'end')")
                lines.append(f"{indent}except: {tgt}.delete('1.0', 'end')")
            elif code == "copy_clip":
                src = p.get("source", "") or "entry"
                lines.append(f"{indent}root.clipboard_clear()")
                lines.append(f"{indent}root.clipboard_append({src}.get())")
            elif code == "save_file":
                src = p.get("source", "") or "entry"
                fn  = p.get("filename", "output.txt")
                lines.append(f"{indent}with open({repr(fn)}, 'w', encoding='utf-8') as _f:")
                lines.append(f"{indent}    _f.write({src}.get())")
            elif code == "open_file":
                lines.append(f"{indent}from tkinter import filedialog as _fd")
                lines.append(f"{indent}_path = _fd.askopenfilename()")
                lines.append(f"{indent}if _path:")
                lines.append(f"{indent}    with open(_path, encoding='utf-8') as _f: _content = _f.read()")
            elif code == "close_window":
                lines.append(f"{indent}root.destroy()")
            elif code == "repeat_n":
                n = p.get("count", "3")
                lines.append(f"{indent}for _i in range(int({repr(n)})):")
                indent += "    "

        if has_timer:
            lines.append(f"    root.after({timer_ms}, on_timer)")
        if not lines:
            lines.append("pass")
    ...
    def _build_exec_globals(self):
        """
        Строит globals для exec.
        Для каждого виджета добавляет его по имени (auto-name = тип+номер).
        Для Checkbutton/Radiobutton гарантированно добавляет _var.
        """
        g = {
            "tk": tk,
            "messagebox": messagebox,
            "root": self.builder.winfo_toplevel(),
        }
        counters = {}
        for info in self.builder.widgets_info:
            wtype  = info.get("type", "Widget")
            # Генерируем имя как тип_N (всегда, даже если info["name"] есть)
            base = wtype.lower()
            counters[base] = counters.get(base, 0) + 1
            auto_name = f"{base}_{counters[base]}"
            # Также используем сохранённое name если есть
            stored_name = info.get("name", "")

            # Получаем реальный виджет — сначала из info["widget"], потом через canvas
            actual = info.get("widget")
            if actual is None:
                win_id = info.get("window_id")
                if win_id:
                    try:
                        actual = self.builder.canvas.nametowidget(
                            self.builder.canvas.itemcget(win_id, "window"))
                    except Exception:
                        pass
            if actual is None:
                continue

            # Регистрируем под auto_name и под stored_name
            g[auto_name] = actual
            if stored_name and stored_name != auto_name:
                g[stored_name] = actual

            # _var для Checkbutton / Radiobutton
            if wtype.lower() in ("checkbutton", "radiobutton"):
                var = getattr(actual, "_var", None)
                if var is None:
                    # Создаём IntVar и привязываем к виджету
                    var = tk.IntVar()
                    try:
                        actual.config(variable=var)
                    except Exception:
                        pass
                    actual._var = var  # кэшируем
                g[f"{auto_name}_var"] = var
                if stored_name:
                    g[f"{stored_name}_var"] = var
    ...
    def _preview_code(self):
        ...
        if not code:
            messagebox.showinfo("Нет блоков", "Добавьте хотя бы один блок.", parent=self)
            return
        win = tk.Toplevel(self)
        win.title("📋 Сгенерированный код")
        win.geometry("640x480")
        win.configure(bg="#0d1117")
        tk.Label(win, text="Сгенерированный Python-код:",
                 font=("Segoe UI", 11, "bold"), bg="#0d1117", fg="#00ff88").pack(
            pady=(10, 4), padx=12, anchor="w")
        txt = tk.Text(win, font=("Consolas", 10), bg="#0d1117", fg="#e0e0e0",
                      insertbackground="white", wrap="none")
        scy = ttk.Scrollbar(win, orient="vertical", command=txt.yview)
        scx = ttk.Scrollbar(win, orient="horizontal", command=txt.xview)
        txt.configure(yscrollcommand=scy.set, xscrollcommand=scx.set)
        ...
        def copy_all():
            ...
            messagebox.showinfo("✅ Скопировано", "Код скопирован!", parent=win)

        tk.Button(win, text="📋 Скопировать", font=("Segoe UI", 10),
                  bg="#0088ff", fg="white", relief="flat",
                  command=copy_all).pack(pady=8)

    # ── применение к виджету ───────────────────────────────────
    def _apply_to_widget(self):
        code = self._generate_code()
        if not code:
            messagebox.showwarning("Нет блоков", "Добавьте хотя бы один блок!", parent=self)
            return
        idx = self.widget_combo.current()
        if idx < 0 or idx >= len(self.builder.widgets_info):
            messagebox.showwarning("Нет виджета", "Выберите целевой виджет!", parent=self)
            return

        info = self.builder.widgets_info[idx]
        info["props"]["block_logic_code"] = code

        # Привязка к реальному виджету с инжекцией всех виджетов в globals
        win_id = info.get("window_id")
        bound_ok = False
        if win_id:
            try:
                actual = self.builder.canvas.nametowidget(
                    self.builder.canvas.itemcget(win_id, "window"))
                # Инжектируем ВСЕ виджеты с холста по именам
                g = self._build_exec_globals()
                exec(code, g)
                fn_names = [l.strip()[4:].split("(")[0]
                            for l in code.split("\n")
                            if l.strip().startswith("def ")]
                if fn_names and fn_names[0] in g:
                    fn = g[fn_names[0]]
                    try:
                        actual.config(command=fn)
                        bound_ok = True
                    except Exception:
                        actual.bind("<Button-1>", fn)
                        bound_ok = True
            except Exception as ex:
                messagebox.showwarning("⚠️ Ошибка привязки",
                    f"Логика сохранена, но не привязана:\n{ex}", parent=self)

        status = "✅ Привязана и активна" if bound_ok else "💾 Сохранена (активна после перезапуска)"
        messagebox.showinfo("✅ Применено!",
            f"Логика ({len(self.program_blocks)} блоков) → "
            f"{info.get('type')} ({info.get('name')})\n"
            f"{status}\n\n"
            f"Виджеты в коде: {list(self._build_exec_globals().keys())[3:]}", parent=self)
