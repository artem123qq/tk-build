# ============================================================
# TkBuilder Ultra v8.7 — Синхронизация props, запуск приложения, правильные цвета в предпросмотре
# Оригинальный код: TkBuilder Ultra v7.2
# Исправления, блочное программирование и доработка: Claude (Anthropic) v8.7
# Дата: 2025
# ============================================================
...
class EnhancedMainWindow(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("TkBuilder Ultra v8.5")
...
#
    def create_main_ui(self):
        ...
        header.pack(pady=30)
#
        tk.Label(header, text="TkBuilder Ultra v8.5",
...
#
        ...
        super().__init__(parent)
        self.parent = parent
        self.title("TkBuilder Ultra v8.5 — Конструктор")
...
#
    def launch_app(self):
        ...
        # Применяем блок-логику и умную логику так же, как в предпросмотре
        self._apply_block_logic_to_preview(app_win, run_widgets)
        self.apply_smart_logic(app_win, run_widgets)
...
#
    ...
    APP_TEMPLATES = [
        {
            "name": "🖱️ Кнопка меняет текст",
            "desc": "Жмёшь кнопку — Label меняет текст. Просто и сразу работает.",
            "preview": ["[Кнопка]", "Текст изменится"],
            "widgets": [
                {"type": "Button", "x": 80,  "y": 80,  "w": 120, "h": 36,
                 "props": {"text": "Нажми меня", "bg": "#0088ff", "fg": "white"}},
                {"type": "Label",  "x": 80,  "y": 140, "w": 200, "h": 36,
                 "props": {"text": "Привет! Нажми кнопку.", "bg": "#f0f0f0", "fg": "#333"}},
            ],
            "logic": [
                # (widget_idx, code)
                (0, "def on_click(event=None):\n    label_1.config(text=\"Кнопка нажата! ✅\")"),
            ]
        },
        {
            "name": "☑️ Флажок включает/выключает текст",
            "desc": "Ставишь галочку — текст меняется сразу. Снимаешь — возвращается.",
            "preview": ["[☑ Флажок]", "Статус: выкл"],
            "widgets": [
                {"type": "Checkbutton", "x": 80, "y": 80,  "w": 160, "h": 36,
                 "props": {"text": "Включить режим", "bg": "white"}},
                {"type": "Label",       "x": 80, "y": 135, "w": 200, "h": 36,
                 "props": {"text": "Статус: выключено", "bg": "#f0f0f0", "fg": "#c00"}},
            ],
            "logic": [
                (0, "def on_click(event=None):\n    v = checkbutton_1_var.get()\n    label_1.config(text=\"Статус: ВКЛЮЧЕНО ✅\" if v else \"Статус: выключено\", fg=\"#00aa00\" if v else \"#cc0000\")"),
            ]
        },
        {
            "name": "✏️ Поле ввода → показать текст",
            "desc": "Пишешь в поле, жмёшь кнопку — текст появляется в Label.",
            "preview": ["[Введи текст...]", "[Показать]", "Результат"],
            "widgets": [
                {"type": "Entry",  "x": 80, "y": 80,  "w": 200, "h": 32,
                 "props": {"bg": "white"}},
                {"type": "Button", "x": 80, "y": 126, "w": 100, "h": 34,
                 "props": {"text": "Показать", "bg": "#0088ff", "fg": "white"}},
                {"type": "Label",  "x": 80, "y": 174, "w": 200, "h": 36,
                 "props": {"text": "Здесь появится текст", "bg": "#f0f0f0"}},
            ],
            "logic": [
                (1, "def on_click(event=None):\n    label_1.config(text=entry_1.get() or \"Поле пустое!\")"),
            ]
        },
        {
            "name": "🔢 Простой счётчик",
            "desc": "Кнопки + и − меняют число в метке.",
            "preview": ["[−]  0  [+]"],
            "widgets": [
                {"type": "Button", "x": 80,  "y": 90, "w": 50,  "h": 50,
                 "props": {"text": "−", "font": "Arial 18 bold", "bg": "#ff5555", "fg": "white"}},
                {"type": "Label",  "x": 145, "y": 90, "w": 70,  "h": 50,
                 "props": {"text": "0", "font": "Arial 20 bold", "bg": "white", "anchor": "center"}},
                {"type": "Button", "x": 230, "y": 90, "w": 50,  "h": 50,
                 "props": {"text": "+", "font": "Arial 18 bold", "bg": "#00aa55", "fg": "white"}},
            ],
            "logic": [
                (0, "def on_click(event=None):\n    try: v=int(label_1.cget(\"text\"))\n    except: v=0\n    label_1.config(text=str(v-1))"),
                (2, "def on_click(event=None):\n    try: v=int(label_1.cget(\"text\"))\n    except: v=0\n    label_1.config(text=str(v+1))"),
            ]
        },
        {
            "name": "🎨 Кнопки меняют цвет фона",
            "desc": "Три кнопки — три цвета. Жмёшь — фон метки меняется.",
            "preview": ["[🔴 Красный]", "[🟢 Зелёный]", "[🔵 Синий]", "[ Цветная рамка ]"],
            "widgets": [
                {"type": "Button", "x": 80, "y": 60,  "w": 110, "h": 34,
                 "props": {"text": "🔴 Красный", "bg": "#ff5555", "fg": "white"}},
                {"type": "Button", "x": 80, "y": 104, "w": 110, "h": 34,
                 "props": {"text": "🟢 Зелёный", "bg": "#00aa55", "fg": "white"}},
                {"type": "Button", "x": 80, "y": 148, "w": 110, "h": 34,
                 "props": {"text": "🔵 Синий",   "bg": "#0088ff", "fg": "white"}},
                {"type": "Label",  "x": 80, "y": 200, "w": 200, "h": 60,
                 "props": {"text": "Выбери цвет", "bg": "#cccccc", "fg": "#333"}},
            ],
            "logic": [
                (0, "def on_click(event=None):\n    label_1.config(bg=\"#ff5555\", text=\"Красный! 🔴\")"),
                (1, "def on_click(event=None):\n    label_1.config(bg=\"#00aa55\", text=\"Зелёный! 🟢\")"),
                (2, "def on_click(event=None):\n    label_1.config(bg=\"#0088ff\", text=\"Синий! 🔵\")"),
            ]
        },
        {
            "name": "🗑️ Кнопка очищает поле",
            "desc": "Entry + кнопка Очистить. Жмёшь — поле пустеет.",
            "preview": ["[Текст в поле...]", "[Очистить ✗]"],
            "widgets": [
                {"type": "Entry",  "x": 80, "y": 90,  "w": 200, "h": 32,
                 "props": {"bg": "white"}},
                {"type": "Button", "x": 80, "y": 136, "w": 110, "h": 34,
                 "props": {"text": "Очистить ✗", "bg": "#ff5555", "fg": "white"}},
            ],
            "logic": [
        ...
        {
            "name": "🪟 Кнопка открывает окно",
            "desc": "Нажимаешь — открывается новое окно с текстом.",
            "preview": ["[Открыть окно]"],
            "widgets": [
                {"type": "Button", "x": 80, "y": 90, "w": 150, "h": 40,
                 "props": {"text": "Открыть окно 🪟", "bg": "#7b1fa2", "fg": "white"}},
            ],
            "logic": [
                (0, "def on_click(event=None):\n    w = tk.Toplevel()\n    w.title(\"Привет!\")\n    w.geometry(\"300x150\")\n    tk.Label(w, text=\"Это новое окно! 👋\", font=(\"Arial\",14)).pack(expand=True)"),
            ]
        },
        {
            "name": "💬 Форма обратной связи",
            "desc": "Имя + сообщение + кнопка Отправить. Показывает подтверждение.",
            "preview": ["Имя:", "[...]", "Сообщение:", "[...]", "[Отправить]"],
            "widgets": [
                {"type": "Label",  "x": 80, "y": 50,  "w": 80,  "h": 28,
                 "props": {"text": "Имя:", "anchor": "w", "bg": "white"}},
                {"type": "Entry",  "x": 80, "y": 80,  "w": 200, "h": 30,
                 "props": {"bg": "white"}},
                {"type": "Label",  "x": 80, "y": 122, "w": 120, "h": 28,
                 "props": {"text": "Сообщение:", "anchor": "w", "bg": "white"}},
                {"type": "Entry",  "x": 80, "y": 152, "w": 200, "h": 30,
                 "props": {"bg": "white"}},
                {"type": "Button", "x": 80, "y": 196, "w": 120, "h": 36,
                 "props": {"text": "Отправить 📨", "bg": "#0088ff", "fg": "white"}},
                {"type": "Label",  "x": 80, "y": 244, "w": 240, "h": 30,
                 "props": {"text": "", "bg": "white", "fg": "#00aa55"}},
            ],
            "logic": [
                (4, "def on_click(event=None):\n    name = entry_1.get()\n    msg = entry_2.get()\n    if name and msg:\n        label_2.config(text=f\"✅ Отправлено, {name}!\")\n    else:\n        label_2.config(text=\"⚠️ Заполни все поля\", fg=\"#ff0000\")"),
            ]
        },
        {
            "name": "🔐 Форма логина",
            "desc": "Имя пользователя + пароль + кнопка Войти. Меняет текст статуса.",
            "preview": ["Логин", "Пароль", "[Войти]", "Статус"],
            "widgets": [
                {"type": "Label",  "x": 80, "y": 60,  "w": 80,  "h": 26,
                 "props": {"text": "Логин:", "anchor": "w", "bg": "white"}},
                {"type": "Entry",  "x": 80, "y": 84,  "w": 200, "h": 28,
                 "props": {"bg": "white"}},
                {"type": "Label",  "x": 80, "y": 120, "w": 80,  "h": 26,
                 "props": {"text": "Пароль:", "anchor": "w", "bg": "white"}},
                {"type": "Entry",  "x": 80, "y": 144, "w": 200, "h": 28,
                 "props": {"bg": "white"}},
                {"type": "Button", "x": 80, "y": 184, "w": 120, "h": 32,
                 "props": {"text": "Войти", "bg": "#0088ff", "fg": "white"}},
                {"type": "Label",  "x": 80, "y": 224, "w": 240, "h": 30,
                 "props": {"text": "Введите логин и пароль", "bg": "white", "fg": "#333"}},
            ],
            "logic": [
                (4, "def on_click(event=None):\n    username = entry_1.get().strip()\n    password = entry_2.get().strip()\n    if username and password:\n        label_2.config(text=f\"Успешный вход: {username}\", fg=\"#008000\")\n    else:\n        label_2.config(text=\"Заполните логин и пароль\", fg=\"#cc0000\")"),
            ]
        },
        {
            "name": "➡ Кнопка показывает текст",
            "desc": "Кнопка + метка. При нажатии меняет текст метки.",
            "preview": ["[Показать сообщение]", "Метка с текстом"],
            "widgets": [
                {"type": "Button", "x": 80, "y": 90,  "w": 180, "h": 34,
                 "props": {"text": "Показать сообщение", "bg": "#4CAF50", "fg": "white"}},
                {"type": "Label",  "x": 80, "y": 140, "w": 220, "h": 34,
                 "props": {"text": "Здесь появится сообщение", "bg": "#f0f0f0", "fg": "#333"}},
            ],
            "logic": [
                (0, "def on_click(event=None):\n    label_1.config(text=\"Кнопка нажата, текст изменён\")"),
            ]
        },
    ]
    ...
    def _apply_app_template(self, tpl, win, clear=False):
        """Расставляет виджеты шаблона на холст с логикой v8.5+"""
        win.destroy()
        ...
        for (w, wtype, x, y, width, height) in new_widgets:
            ...
            auto_name = f"{base}_{tpl_place_counters[base]}"
            # Маленький тег-лейбл над виджетом
...
#
            if self.snap_to_grid.get():
                ...
                y = (y // self.grid_size.get()) * self.grid_size.get()
#
            print(f"Клик на холсте: {x}, {y}, выбран тип: {self.selected_widget_type}")  # Отладка
#
...
# Claude (Anthropic) v7.5 — полная переработка
# ================================================================
...
class BlockLogicEditor(tk.Toplevel):
    """Редактор блочной логики v7.5 — полная переработка"""

    def __init__(self, builder):
        super().__init__(builder)
        self.builder = builder
        self.title("🧩 Блочное программирование — TkBuilder v7.5")
